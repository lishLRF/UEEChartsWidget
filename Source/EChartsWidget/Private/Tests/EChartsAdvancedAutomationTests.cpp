...
bool FEChartsLegendSettingsLifecycleTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Unacknowledged legend candidate does not replace last-good font"), Widget->LegendSettings.FontSize, 12);
	...
	Widget->OnConsoleMessage.Broadcast(FString::Printf(TEXT("__UE_ECHARTS_LEGEND_RESULT__:1:%lld:0:first failed"), FirstRequest), FString(), 0);
	TestEqual(TEXT("Failed legend candidate preserves last-good font"), Widget->LegendSettings.FontSize, 12);
	const int64 LatestRequest = Widget->GetPendingLegendRequestIdForTesting();
	TestTrue(TEXT("Latest legend setting follows failed first ACK"), LatestRequest > 0 && LatestRequest != FirstRequest);
	...
	TestEqual(TEXT("Successful legend ACK commits latest clamped font"), Widget->LegendSettings.FontSize, 20);
	TestEqual(TEXT("Successful legend ACK commits clamped item gap"), Widget->LegendSettings.ItemGap, 0);
	TestEqual(TEXT("Successful legend ACK commits clamped item width"), Widget->LegendSettings.ItemWidth, 1);
	TestEqual(TEXT("Successful legend ACK commits clamped item height"), Widget->LegendSettings.ItemHeight, 100);
	TestEqual(TEXT("Successful legend ACK commits clamped custom X"), Widget->LegendSettings.CustomXPercent, 100.0f);
	TestEqual(TEXT("Successful legend ACK commits clamped custom Y"), Widget->LegendSettings.CustomYPercent, 0.0f);
	...
	TestEqual(TEXT("Unacknowledged reset preserves last-good position"), Widget->LegendSettings.Position, EEChartsLegendPosition::Custom);
	TestEqual(TEXT("Unacknowledged reset preserves last-good font"), Widget->LegendSettings.FontSize, 20);
	const int64 ResetRequest = Widget->GetPendingLegendRequestIdForTesting();
	Widget->OnConsoleMessage.Broadcast(FString::Printf(TEXT("__UE_ECHARTS_LEGEND_RESULT__:2:%lld:1:reset"), ResetRequest), FString(), 0);
	TestEqual(TEXT("Reset ACK restores default legend position"), Widget->LegendSettings.Position, EEChartsLegendPosition::Auto);
	TestEqual(TEXT("Reset ACK restores default font size"), Widget->LegendSettings.FontSize, 12);

	Settings.FontSize = 30;
	Widget->SetLegendSettings(Settings);
	const int64 PreReleaseRequest = Widget->GetPendingLegendRequestIdForTesting();
	Settings.FontSize = 40;
	Widget->SetLegendSettings(Settings);
	Widget->ReleaseSlateResources(false); Widget->PrepareRebuildForTesting();
	Widget->OnConsoleMessage.Broadcast(FString::Printf(TEXT("__UE_ECHARTS_LEGEND_RESULT__:2:%lld:1:stale"), PreReleaseRequest), FString(), 0);
	TestEqual(TEXT("Old generation legend ACK is ignored"), Sink->LegendResultCount, 5);
	Widget->OnConsoleMessage.Broadcast(TEXT("__UE_ECHARTS_READY__:3"), FString(), 0);
	const int64 MigratedRequest = Widget->GetPendingLegendRequestIdForTesting();
	TestTrue(TEXT("Release migrates the latest requested legend to a new request"), MigratedRequest > 0 && MigratedRequest != PreReleaseRequest);
	TestEqual(TEXT("Only latest requested legend is replayed"), Widget->GetInFlightLegendSettingsForTesting().FontSize, 40);
	TestEqual(TEXT("Migrated request does not replace last-good before ACK"), Widget->LegendSettings.FontSize, 12);
	Sink->LegendFailureInitializeWidget = Widget;
	Widget->OnConsoleMessage.Broadcast(FString::Printf(TEXT("__UE_ECHARTS_LEGEND_RESULT__:3:%lld:0:replay failed"), MigratedRequest), FString(), 0);
